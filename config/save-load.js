...
function buildCurrentConfig() {
  ...
  Object.keys(screens).forEach(screenId => {
    ...
    // Safely convert customSocaAssignments
    let socaAssignmentsArray = [];
    if(data.customSocaAssignments instanceof Map) {
      socaAssignmentsArray = Array.from(data.customSocaAssignments.entries());
    } else if(Array.isArray(data.customSocaAssignments)) {
      socaAssignmentsArray = data.customSocaAssignments;
    }
    ...
    // Safely convert customDataDestinations (values are { proc, box } objects)
    let dataDestinationsArray = [];
    if(data.customDataDestinations instanceof Map) {
      dataDestinationsArray = Array.from(data.customDataDestinations.entries());
    } else if(Array.isArray(data.customDataDestinations)) {
      dataDestinationsArray = data.customDataDestinations;
    }
    ...
    screensData[screenId] = {
      ...
      data: {
        ...
        customDataDestinations: dataDestinationsArray,
...
function applyConfiguration(config, fileNameOverride) {
  ...
  if(config.version === '2.0' && config.screens) {
    ...
    safeScreenIds.forEach(screenId => {
      ...
      // Safely restore customSocaAssignments (validate [key, value] pairs)
      let socaAssignmentsMap = new Map();
      if(savedScreen.data && Array.isArray(savedScreen.data.customSocaAssignments)) {
        const validEntries = savedScreen.data.customSocaAssignments.filter(e => Array.isArray(e) && e.length === 2);
        socaAssignmentsMap = new Map(validEntries);
      }
      ...
      // Safely restore customDataDestinations (validate [key, {proc, box}] pairs)
      let dataDestinationsMap = new Map();
      if(savedScreen.data && Array.isArray(savedScreen.data.customDataDestinations)) {
        savedScreen.data.customDataDestinations.forEach(e => {
          if(!Array.isArray(e) || e.length !== 2) return;
          const v = e[1];
          if(!v || typeof v !== 'object' || Array.isArray(v)) return;
          const proc = (typeof v.proc === 'number' && v.proc >= 1) ? v.proc : null;
          const box = (typeof v.box === 'number' && v.box >= 1) ? v.box : null;
          if(proc === null && box === null) return;
          dataDestinationsMap.set(e[0], { proc: proc, box: box });
        });
      }
      ...
      screens[screenId] = {
        ...
        data: {
          ...
          customSocaAssignments: socaAssignmentsMap,
          customDataLineAssignments: dataLineAssignmentsMap,
          customDataDestinations: dataDestinationsMap
        },
    ...
    customDataDestinations = new Map(config.customDataDestinations || []);

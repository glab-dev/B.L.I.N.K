...
/**
 * Comprehensive Test: Power Auto-Population
 * Tests that max panels per circuit auto-populates based on voltage/breaker/power settings.
 */
test.describe('Power Auto-Population @comprehensive @desktop', () => {
  ...
  test('should display auto-calculated Max/Circuit placeholder', async ({ page }) => {
    const placeholder = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');
  ...
  test('should update Max/Circuit placeholder when voltage changes', async ({ page, power }) => {
    const placeholder208 = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');

    await power.setVoltage(120);
    await page.waitForTimeout(500);

    const placeholder120 = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');
  ...
  test('should update Max/Circuit placeholder when breaker changes', async ({ page, power }) => {
    const placeholder20 = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');

    await power.setBreaker(30);
    await page.waitForTimeout(500);

    const placeholder30 = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');
  ...
  test('should update Max/Circuit placeholder when switching Max/Avg power', async ({ page, power }) => {
    const placeholderMax = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');

    await power.setPowerType('avg');
    await page.waitForTimeout(500);

    const placeholderAvg = await page.locator('#maxPanelsPerCircuit').getAttribute('placeholder');
  ...
  test('should display auto-calculated Max/Data placeholder', async ({ page }) => {
    const placeholder = await page.locator('#maxPanelsPerData').getAttribute('placeholder');
    expect(placeholder).toBeTruthy();
    expect(Number(placeholder)).toBeGreaterThan(0);
  });
});

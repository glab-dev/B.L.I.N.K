...
// Build a section-masked PDF blob from a canvas cache that was already captured.
// Export All emits one PDF per screen per view, so the expensive capture pass
// runs twice for the whole project instead of once per file.
function _buildSectionPdfFromCache(opts, canvasCache, callback) {
  try {
    if (!window.pdfMake || typeof buildComplexPdf !== 'function') { callback(null); return; }
    var docDef = buildComplexPdf(opts, canvasCache || {});
    pdfMake.createPdf(docDef).getBlob(function(blob) { callback(blob); });
  } catch(e) {
    console.error('_buildSectionPdfFromCache error:', e);
    callback(null);
  }
}

...
/**
 * Comprehensive Mobile Test: Panel Deletion Cascade
 ...
 */
test.describe('Panel Deletion Cascade @comprehensive @mobile', () => {
  ...
  test('should restore deleted panel via undo', async ({ page }) => {
    ...
    // Undo (use evaluate since #undoBtn may not be visible on mobile)
  ...
  test('should handle deleting all panels in a column', async ({ page }) => {
    ...
    // Wait for the layout to render and currentPw/currentPh to be set
    await page.waitForFunction(
      () => typeof currentPw !== 'undefined' && currentPw > 0,
      null,
      { timeout: 5000 }
    ).catch(async () => {
      // If currentPw not yet set, force a calculate cycle
    ...
    // Use the input values for expected total rather than relying on currentPw/currentPh
    const pw = await page.evaluate('parseInt(document.getElementById("panelsWide").value) || 0');
    const ph = await page.evaluate('parseInt(document.getElementById("panelsHigh").value) || 0');
    const deleted = await page.evaluate('deletedPanels.size');
    expect(pw).toBe(4);
    expect(ph).toBe(3);
    expect(pw * ph - deleted).toBe(9);

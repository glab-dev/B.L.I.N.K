...
/**
 * Comprehensive Mobile Test: Panel Type Cascading Effects
 ...
 */
test.describe('Panel Type Cascading Effects @comprehensive @mobile', () => {
  ...
  test('should show connecting plates choice for CB5 MKII', async ({ page, dimensions }) => {
    ...
    const connectingPlatesSection = page.locator('#connectingPlatesSection');
    await connectingPlatesSection.scrollIntoViewIfNeeded();
    await expect(connectingPlatesSection).toBeVisible();
  ...
  test('should switch between Air Frame and Plates for CB5', async ({ page, dimensions }) => {
    ...
    const platesBtn = page.locator('#connectionPlatesBtn');
    await platesBtn.scrollIntoViewIfNeeded();

    // Switch to Plates
    await platesBtn.click();
    await page.waitForTimeout(200);
    await expect(platesBtn).toHaveClass(/active/);
  ...
  test('should auto-set structure to Floor for BM4 Matte', async ({ page, dimensions, structure }) => {
    ...
    await structure.structureTypeSelect.scrollIntoViewIfNeeded();
  ...
  test('should update Max/Circuit placeholder when panel type changes', async ({ page, dimensions }) => {
    ...
    const placeholder1 = await maxPanelsPerCircuit.getAttribute('placeholder');

    await dimensions.panelTypeSelect.selectOption('INFILED_AMT8_3');
    await page.waitForTimeout(500);

    const placeholder2 = await maxPanelsPerCircuit.getAttribute('placeholder');
  ...
  test('should update Max/Data placeholder when panel type changes', async ({ page, dimensions }) => {
    ...
    const placeholder1 = await maxPanelsPerData.getAttribute('placeholder');

    await dimensions.panelTypeSelect.selectOption('CB5_MKII');
    await page.waitForTimeout(500);

    const placeholder2 = await maxPanelsPerData.getAttribute('placeholder');

...
/**
 * Comprehensive Mobile Test: NovaStar MX40 Pro Mode & All Processors
 * Tests NovaStar MX40 Direct/Indirect mode toggle and iterates through all processors.
 */
test.describe('NovaStar MX40 Pro & All Processors @comprehensive @mobile', () => {
  ...
  test('should show MX40 mode toggle when NovaStar MX40 Pro selected', async ({ page, data }) => {
    const processorSelect = page.locator('#processor');
    await processorSelect.scrollIntoViewIfNeeded();
    await data.processorSelect.selectOption('NovaStar_MX40_Pro');
    await page.waitForTimeout(300);

    const mx40ModeToggleRow = page.locator('#mx40ModeToggleRow');
    await mx40ModeToggleRow.scrollIntoViewIfNeeded();
    await expect(mx40ModeToggleRow).toBeVisible();
  ...
  test('should hide MX40 mode toggle for non-MX40 processors', async ({ page, data }) => {
    ...
    await page.waitForTimeout(300);
    await expect(mx40ModeToggleRow).not.toBeVisible();
  });
  ...
  test('should switch between Direct and Indirect modes', async ({ page, data }) => {
    ...
    const mx40IndirectBtn = page.locator('#mx40IndirectBtn');
    await mx40IndirectBtn.scrollIntoViewIfNeeded();

    // Switch to Indirect
    await mx40IndirectBtn.click();
    await page.waitForTimeout(200);
    await expect(mx40IndirectBtn).toHaveClass(/active/);
    await expect(page.locator('#mx40DirectBtn')).not.toHaveClass(/active/);

    // Switch back to Direct
    const mx40DirectBtn = page.locator('#mx40DirectBtn');
    await mx40DirectBtn.click();
    await page.waitForTimeout(200);
    await expect(mx40DirectBtn).toHaveClass(/active/);
    await expect(mx40IndirectBtn).not.toHaveClass(/active/);
  });
  ...
  test('should iterate through all frame rates and bit depths', async ({ page, data }) => {
    ...
    for (const fr of frameRates) {
      await data.frameRateSelect.scrollIntoViewIfNeeded();
    ...
    for (const bd of bitDepths) {
      await data.bitDepthSelect.scrollIntoViewIfNeeded();

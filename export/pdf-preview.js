...
function buildPageModel() {
  ...
  screenIds.forEach((screenId, sIdx) => {
    ...
    if (opts.specs || opts.gearList) {
      ...
      // First screen: add title and timestamp (matching pdf.js page 1)
      if (sIdx === 0) {
        specsPage.elements.push({
          id: 'title',
          type: 'text',
          text: 'B.L.I.N.K. Report',
          x: PP_MARGIN, y: headerY, w: 120, h: 8,
        ...
        specsPage.elements.push({
          ...
          x: ppPageWidth - PP_MARGIN - 80, y: headerY, w: 80, h: 5,
          fontSize: 9, align: 'right', color: '#666'
        });
        headerY += 10;
        specsPage.elements.push({
          id: 'screen_count',
          type: 'text',
          text: 'Total Screens: ' + screenIds.length,
          x: PP_MARGIN, y: headerY, w: 80, h: 5,
      ...
      specsPage.elements.push({
        id: screenId + '_header',
        type: 'text',
        text: 'Screen ' + (sIdx + 1) + ': ' + screen.name,
...
function drawPreviewElement(ctx, el, scale, pageIndex) {
  ...
  switch (el.type) {
    ...
    case 'textblock':
      ...
      ctx.strokeStyle = isSelected ? '#2196F3' : (isOverlapping ? '#ff9800' : '#e0e0e0');
    ...
    case 'layout':
      ctx.save();
      // Draw title text at top of element
      ctx.font = 'bold ' + Math.max(8, el.fontSize * s * 0.35) + 'px Arial';
      ctx.fillStyle = '#000';
      ctx.textBaseline = 'top';
      ctx.textAlign = 'left';
...
function exportFromPreview() {
  ...
  // Apply options to legacy pdfExportOptions
  ...
  // Set print modes AFTER close (close resets them)
  ecoPrintMode = opts.ecoFriendly;
  greyscalePrintMode = opts.greyscale;

  // Use the existing exportPDF() with page format/orientation
  exportPDF();
}
